  ...
  class ThorinGcloudStore extends thorin.Interface.Store {
    ...
    init(storeConfig) {
      this[config] = thorin.util.extend({
        ...
        path: path.normalize(thorin.root + '/app/models')
      }, storeConfig);
      if (process.env.GOOGLE_APPLICATION_CREDENTIALS && (!this[config].credentials || Object.keys(this[config].credentials).length === 0)) {
